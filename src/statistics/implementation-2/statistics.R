...
files <- list.files(path="out", pattern="", full.names=TRUE, recursive=TRUE)
...
for (algo in files) {
  ...
  file.create(paste0("src/statistics/implementation-2/stats/", basename(algo)))
  write.table(result, file=paste0("src/statistics/implementation-2/stats/", basename(algo)), row.names=FALSE, quote=FALSE)
}

# Get all the results from previous steps
results <- list.files(path="src/statistics/implementation-2/stats", full.names=TRUE, recursive=TRUE)
...
write.table(t_tests, file="src/statistics/implementation-2/results/t_test", row.names=FALSE, quote=FALSE)
write.table(wilcox_tests, file="src/statistics/implementation-2/results/wilcox_test", row.names=FALSE, quote=FALSE)
